...
async function authorPresent(name) {
  ...
  let found = false;
  rows.forEach((writer) => {
    if (writer.author === name) found = true;
  });
  return found;
}
...
async function getAuthorId(name) {
  const result = await db.query(
    "SELECT id FROM author WHERE LOWER(author) = $1",
    [name],
  );
...
app.get("/book/:id", async (req, res) => {
  ...
  const col =
    "author.id, author, book.id, title, isbn, rating, TO_CHAR(dateread, 'dd/mm/yyyy'), review, authorid";
...
app.get("/rating", async (req, res) => {
  const result = await db.query(
    "SELECT * FROM book WHERE userid = $1 ORDER BY rating DESC",
    [req.user.id],
  );
...
app.get("/recency", async (req, res) => {
  const result = await db.query(
    "SELECT * FROM book WHERE userid = $1 ORDER BY dateread DESC",
    [req.user.id],
  );
  const lib = result.rows;
...
app.get("/index", async (req, res) => {
  ...
  if (req.isAuthenticated()) {
    try {
      ...
      const result = await db.query("SELECT * FROM book WHERE userid = $1", [
        req.user.id,
      ]);
      const lib = result.rows;
...
app.post("/add", async (req, res) => {
  ...
  try {
    ...
    if (!(await authorPresent(authorName))) {
      const authorResult = await db.query(
        "INSERT INTO author (author) VALUES ($1) RETURNING id",
        [authorName],
      );
  ...
  try {
    ...
    await db.query(
      "INSERT INTO book (title, isbn, rating, dateread, review, authorid, userid) VALUES ($1,$2,$3,$4,$5,$6,$7)",
...
app.post("/edit", async (req, res) => {
  ...
  try {
    await db.query("UPDATE book SET review = $1 WHERE id = $2", [
      newReview,
      id,
    ]);
    res.redirect("back");
